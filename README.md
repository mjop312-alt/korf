...
| `npm run setup-search` | eenmalig: pg_trgm-uitbreiding + trigram-index aanzetten (nodig voor typo-tolerant zoeken) |
...
| `lib/crawl/jumbo.ts` | **Jumbo** — GraphQL per hoofdcategorie, met EAN, categoriepad en echte actie-data. Een falende pagina wordt 3× herhaald; blijft die falen (één kapot product maakt elk venster van 24 onbruikbaar), dan worden de aangrenzende vensters geprobeerd, zodat alleen het kapotte product zelf ontbreekt. |
